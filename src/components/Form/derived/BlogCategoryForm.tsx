...
import { $_create_category, $_edit_category, reqSucceed, toLocalDate, toLocalTime } from 'utils'
...
export function BlogCategoryForm(props: BlogCategoryFormProps) {
  ...
  async function onSubmit(form: any) {
    ...
    if (reqSucceed(response)) {
      toast.success(`${props.defaultValues ? 'ویرایش' : 'ساخت'} دسته بندی با موفقیت انجام شد`)
      reset()
      props.callback && props.callback()
    } else {
      toast.error(`${props.defaultValues ? 'ویرایش' : 'ساخت'} دسته بندی موفقیت آمیز نبود`)
    }
    toggleLoading('submission')
  }
    ...
    <Form onSubmit={handleSubmit(onSubmit)}>
      ...
      <InputGroup col>
        ...
        {props.readOnly ? (
          ...
          <input {...register('content', { required: true })} placeholder="محتوا" />
        )}
      </InputGroup>
      <InputGroup col>
        <label>اولویت</label>
      ...
      {props.readOnly && (
        <>
          ...
          <InputGroup col>
            ...
            <Value>
              {toLocalDate(props.defaultValues?.created_at)} - {toLocalTime(props.defaultValues?.created_at)}
            </Value>
          </InputGroup>
          <InputGroup col>
            <label>تاریخ آخرین بروزرسانی</label>
            <Value>
              {toLocalDate(props.defaultValues?.updated_at)} - {toLocalTime(props.defaultValues?.updated_at)}
            </Value>
          </InputGroup>
        </>
      )}
      ...
      {!props.readOnly && (
        <SubmitButton status={props.defaultValues ? 'Info' : 'Success'} disabled={loadingState.includes('submission')}>
