...
export const SearchCommentsPage = () => {
  ...
  const { data: comments, refetch: refetchComments } = useQuery(["comments", router.query], () =>
    search_in("comments", router.query, router.query, Cookies.get(process.env["TOKEN"]!)),
  );
  const commentFields = useQuery(
    ["commentFields"],
    () =>
      new Promise((resolve) =>
        resolve({
  ...
  return (
    <Layout title="نظرات">
      ...
      {has(permissions, PermissionEnum.browseComment) && (
        <>
          <SearchBar
            fields={(commentFields?.data as any)?.fields}
            entity="comments"
            params={router.query}
            callback={(form: any) =>
              router.push({
