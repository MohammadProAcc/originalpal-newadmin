...
export const EditCommentPage: React.FC = () => {
  ...
  const checkToggle = async (commentId: number, admin_check: 1 | 0) => {
    ...
    if (response?.status === "success") {
      ...
      toast.success(
        `نظر ${commentId} ${admin_check ? "تایید" : "سلب تایید"} شد`,
      );
    } else {
  ...
  return (
    <Layout title={`${comment?.id}`}>
      <h1 style={{ marginBottom: "4rem" }}>
        نظر {comment?.id}, {comment?.product_id ? "محصول " : "مقاله "}{" "}
        {comment?.product_id ?? comment?.blog_id}
        <FlexContainer style={{ display: "inline-flex" }}>
          ...
          <HeaderButton
            status="Danger"
            onClick={() => setItemToRemove(comment)}
          >
            حذف
          </HeaderButton>
        </FlexContainer>
