...
export const Banners = () => {
  ...
  const data = banners?.data?.data?.map((banner: any) => [
    banner?.id,
    <Avatar
      src={`${process.env.SRC}/${banner?.media ? banner?.media[0]?.u : null}`}
    />,
    banner?.title,
    ...
    <Container>
      ...
      {has(permissions, PermissionEnum.deleteStand) && (
        <Button
          status="Danger"
          onClick={() => setItemToRemove(banner)}
        >
          حذف
        </Button>
      )}
  ...
  return (
    <Layout title="بنر های ایستاده">
      ...
      <FlexContainer>
        ...
        {tableSelections?.length > 0 &&
          ...
          <HeaderButton
            status="Danger"
            appearance="outline"
            onClick={() => setItemsToRemove(tableSelections)}
          >
      ...
      <Modal on={itemToRemove} toggle={toggleModal}>
        <ModalBox fluid>
          آیا از حذف بنر شماره{" "}
          <span className="text-danger">{itemToRemove?.id}</span> با عنوان{" "}
          <span className="text-danger">{itemToRemove?.title}</span>{" "}
          اطمینان دارید؟
          <ButtonGroup>
      ...
      <Modal on={itemsToRemove} toggle={togglePluralRemoveModal}>
        <ModalBox fluid>
          ...
          <ButtonGroup>
            <Button
              onClick={togglePluralRemoveModal}
              style={{ marginLeft: "1rem" }}
            >
              خیر، منصرم شدم
            </Button>
            <Button
              onClick={() => pluralRemoveTrigger(tableSelections)}
              disabled={loading}
              status="Danger"
            >
              بله، حذف شوند
